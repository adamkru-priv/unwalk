...
Deno.serve(async (req) => {
  ...
  // Handle preflight FIRST (before any logic that might fail)
  if (req.method === "OPTIONS") {
    return new Response(null, { status: 204, headers: corsHeaders });
  }

  try {
    // ✅ Initialize Resend inside handler (after OPTIONS check)
    ...
    const { data, error } = await resend.emails.send({
      from: "MOVEE <noreply@mail.movee.one>", // ✅ Używaj TWOJEJ zweryfikowanej domeny
      to: recipientEmail,
