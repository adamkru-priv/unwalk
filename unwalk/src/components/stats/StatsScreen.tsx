...
type StatsScreenProps = {
  /** When true, StatsScreen renders only its main content (no AppHeader / BottomNavigation). */
  embedded?: boolean;
};

export function StatsScreen({ embedded = false }: StatsScreenProps) {
  ...
  const visibleChallenges = useMemo(() => {
    ...
    return completedChallenges.slice(0, 5);
  }, [completedChallenges, showAllHistory]);
  ...
  return (
    <div className={embedded ? 'text-gray-900 dark:text-white font-sans' : 'min-h-screen bg-gray-50 dark:bg-[#0B101B] text-gray-900 dark:text-white pb-20 font-sans'}>
      {!embedded && (
        <AppHeader />
      )}
      ...
      <main className={embedded ? 'space-y-6' : 'px-5 py-6 max-w-4xl mx-auto space-y-6'}>
        ...
        <section className="bg-white dark:bg-[#151A25] border border-gray-200 dark:border-white/5 rounded-2xl p-5">
          <h2 className="text-sm font-semibold text-gray-900 dark:text-white mb-4 flex items-center gap-2">
            ...
            {firstChallengeDate && (
              <span className="text-[11px] text-gray-500 dark:text-white/40 font-normal ml-auto">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="bg-gray-50 dark:bg-[#0B101B] border border-gray-200 dark:border-white/5 rounded-xl p-4 text-center">
              <div className="text-xl font-extrabold text-blue-600 dark:text-blue-400 mb-1">
                {totalSteps.toLocaleString()}
              </div>
              <div className="text-[11px] text-gray-600 dark:text-white/60">Total Steps</div>
            </div>
            <div className="bg-gray-50 dark:bg-[#0B101B] border border-gray-200 dark:border-white/5 rounded-xl p-4 text-center">
              <div className="text-xl font-extrabold text-green-600 dark:text-green-400 mb-1">
                {totalDistance}km
              </div>
              <div className="text-[11px] text-gray-600 dark:text-white/60">Distance</div>
            </div>
            <div className="bg-gray-50 dark:bg-[#0B101B] border border-gray-200 dark:border-white/5 rounded-xl p-4 text-center">
              <div className="text-xl font-extrabold text-purple-600 dark:text-purple-400 mb-1">
                {completedCount}
              </div>
              <div className="text-[11px] text-gray-600 dark:text-white/60">Completed</div>
            </div>
            <div className="bg-gray-50 dark:bg-[#0B101B] border border-gray-200 dark:border-white/5 rounded-xl p-4 text-center">
              <div className="text-xl font-extrabold text-orange-600 dark:text-orange-400 mb-1">
                {totalDays > 0 ? `${totalDays}d ${totalHours}h` : totalHours > 0 ? `${totalHours}h` : '< 1h'}
              </div>
              <div className="text-[11px] text-gray-600 dark:text-white/60">Active Time</div>
            </div>
          </div>
          ...
          {!isGuest && totalPoints > 0 && (
            <div className="mt-3 bg-gradient-to-r from-amber-50 to-orange-50 dark:from-amber-950/30 dark:to-orange-950/30 border border-amber-300 dark:border-amber-500/30 rounded-xl p-3 text-center">
              ...
              <div className="text-[11px] text-amber-700 dark:text-amber-500 font-semibold">
                Total Points Earned
              </div>
            </div>
          )}

          <p className="text-[11px] text-gray-500 dark:text-white/40 text-center mt-3">
        ...
        <section className="bg-white dark:bg-[#151A25] border border-gray-200 dark:border-white/5 rounded-2xl p-5">
          <div className="flex items-center justify-between gap-3 mb-4">
            ...
            {!loading && completedChallenges.length > 5 && (
              <button
      ...
      {/* Bottom Navigation */}
      {!embedded && <BottomNavigation currentScreen="home" />}
    </div>
  );
}
