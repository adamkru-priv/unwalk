...
export function AccountTypeCards({ isGuest, userTier, onTierChange, onShowAuthModal }: AccountTypeCardsProps) {
  ...
  useEffect(() => {
    ...
    const currentCardId = isGuest ? 'guest-card' : userTier === 'basic' ? 'basic-card' : 'pro-card';
  ...
  return (
    <section className="bg-white dark:bg-[#151A25] rounded-2xl p-5 shadow-sm border border-gray-100 dark:border-white/5">
      ...
      {/* Visual hint that more cards are available */}
      <div className="flex items-center justify-center gap-1 mt-3">
        <div className={`h-1.5 w-1.5 rounded-full transition-all ${isGuest ? 'bg-blue-500 w-4' : 'bg-gray-300 dark:bg-white/20'}`}></div>
        <div className={`h-1.5 w-1.5 rounded-full transition-all ${!isGuest && userTier === 'basic' ? 'bg-green-500 w-4' : 'bg-gray-300 dark:bg-white/20'}`}></div>
        <div className={`h-1.5 w-1.5 rounded-full transition-all ${!isGuest && userTier === 'pro' ? 'bg-amber-500 w-4' : 'bg-gray-300 dark:bg-white/20'}`}></div>
      </div>
    </section>
  );
}
